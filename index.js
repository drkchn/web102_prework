...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++) {
        ...
        game.innerHTML = `<img src="${games[i].img}" alt="${games[i].name}" class="game-img"> <div>${games[i].name} has ${games[i].backers} backers! <br><br> ${games[i].description}</div>`;
...
const moneyPledged = GAMES_JSON.reduce ( (total, game) => {
...
const displayStrOneUnfunded = `A total of $${moneyPledged.toLocaleString("en-US")} has been raised for ${numGames} games. Currently, 1 game remains unfunded. We need your help to fund these amazing games!`;

const displayStrMoreUnfunded = `A total of $${moneyPledged.toLocaleString("en-US")} has been raised for ${numGames} games. Currently, ${numUnfundedGames} games remain unfunded. We need your help to fund these amazing games!`;

// create a string that explains the number of unfunded games using the ternary operator
const message = numUnfundedGames === 1 ? displayStrOneUnfunded : displayStrMoreUnfunded;
